...
class SephoraSpider(scrapy.Spider):
    ...
    def parse_item_page(self, response):
        ...
        if len(response.xpath('//div[@class="css-8tl366"]') >= 2):
            ingredients = response.xpath('//div[@class="css-8tl366"]')[2].xpath('./text()').extract()
        else:
            ingredients = 'None'

        name = response.xpath('//span[@class="css-1g2jq23"]/text()').extract()
        price = response.xpath('//div[@class="css-18suhml"]/text()').extract()
        brand = response.xpath('//a[@class="css-zvvfrv"]/span[@class="css-cjz2sh"]/text()').extract()
        ...
        if len(response.xpath('//div[@class="css-12alag6"]/a[@class="css-u2mtre"]/text()')) > 1:
            ...
            sub_category = response.xpath('//div[@class="css-1lb5emk"]/a[@class="css-1i9riiu"]/text()').extract()
        else:
